...
![The $\K$ Interdependence System: four coupled degree dimensions emerging from the bipartite actor-component network.](figures/cd2026_conceptual_architecture.png){width=85%}
...
Throughout this paper we adopt the following notation.  The bipartite incidence
...
![Simulated demonstration that the four $\K$ dimensions move as a coupled system.  A single seeded run of 20 actors over 24 components with a block-diagonal influence matrix, plotted by \pkg{searchnet} from the simulated chain.  Because all four degrees are read off the same bipartite incidence matrix, they rise together from an empty start; the smallest pairwise correlation across the four series is $0.99$.  This panel is simulated output, not empirical data.  Generated by \code{paper/replication/make\_k\_system\_figures.R}.](figures/fig_k_coupling_sim.png){width=80%}
...
Ising--logit correspondence.

### Ergodicity: forgetting the initial condition {#sec:ergodicity}

Theorem~4 asserts that the chain's stationary distribution is unique and
independent of where the simulation starts.  That property is easy to
illustrate badly.  Running the model once from a sparse start and once from a
dense start, then observing that the two final densities are close, establishes
very little: no threshold was declared in advance, a single run per arm supplies
no sampling distribution, and---most importantly---an ergodic chain forgets its
initial condition *at a rate*, which a pair of endpoints discards.

\pkg{searchnet} therefore ships the demonstration as a measurement.
\code{searchnet\_ergodicity\_sweep()} runs the same fixed-coefficient model from
two contrasting starting densities across a range of chain lengths, with
independent replicates at each, and reports how fast the between-arm gap decays:

```{r ergodicity, cache=TRUE, fig.height=5, fig.width=10, fig.cap="Independence from initial conditions, measured rather than asserted. (a) Two arms starting at density 0.10 and 0.80 converge as run length grows; points are replicates. (b) The between-arm gap decays as a power of run length on log--log axes. The dashed line is the declared equivalence margin; the shaded band is the Monte Carlo floor, below which the measured gap reflects replicate noise rather than mixing, and which is excluded from the fitted slope."}
...
iterations per actor, a collapse of more than 99\%, and the gap decays as a
power of run length with slope near $-1$ and $R^2 > 0.9$.  The verdict is
computed rather than printed: at the longest run length the arms are compared by
a two-one-sided-tests equivalence procedure against a margin fixed in the call,
so the function is able to report *non*-equivalence, and does so at the shorter
run lengths.  This is what makes the sweep evidence rather than illustration.

Two details matter for anyone reproducing this.  The Monte Carlo floor is drawn
explicitly, because once the arms are genuinely equivalent the measured gap
settles at the expected difference between two sample means from the same
distribution rather than falling to zero; fitting a decay slope through those
points would overstate the rate.  And the grid is sized so that density has
resolution: on the 4 $\times$ 5 grid used in earlier versions of this
demonstration a single tie moved the density by 0.05, which is coarser than any
equivalence margin worth declaring.

The vignette also provides computational verification that the SAOM Markov chain
satisfies detailed balance with respect to the Gibbs distribution, which is the
mechanism underlying the convergence measured here.
...
![Simulated demonstration that a shock relocates the coupled system.  Three seeded arms share starting conditions and the random seed, so their trajectories coincide exactly until the intervention at the dotted line.  A sparsity shock (density $-5.0 \rightarrow -6.5$) moves every dimension down; a popularity shock ($0.15 \rightarrow 1.20$) moves every dimension up.  Terminal mean $K_{CC}$ is $10.7$, $14.1$ and $21.6$ in the sparsity, control and popularity arms.  This panel is simulated output, not empirical data.  Generated by \code{paper/replication/make\_k\_system\_figures.R}.](figures/fig_k_shock_relocation.png){width=80%}
...
The package includes six predefined scenarios:
...
\item \textbf{Epistasis rewiring}: Modifying the component influence
...
<!-- TODO (2026-09-09) AUTHOR DECISION REQUIRED -- DEANONYMIZATION RISK.
     This subsection still reports the empirical and simulation results of the
     unpublished companion manuscript (the 4.8-fold overlap series, the
     channel-deletion table, the 54-cell robustness sweep) in a paper that
     publishes under the author's name while the companion goes to blind
     review.  The figure that accompanied it has been removed; the numbers
     have not, because replacing them takes more than a mechanical edit.
     Two replacement framings, both of which keep the transferable software
     lessons (the three-layer conformance split and the path-integral probe)
     and drop the empirical payload, are drafted in
     paper/_drafts/2026-09-09_app-airlines.md.  Choose one, or delete the
     subsection and the matching row in the applications table above. -->
...
<!-- TODO (2026-09-09): figure figures/cd2026_kaa_trajectory.png removed here.
     It was an empirical series from an unpublished companion manuscript now
     under blind review, so it is neither reproducible from this replication
     bundle nor safe to publish under the author's name.  See the note below
     on the rest of this subsection. -->
...
mod <- saomnk_model(
  ...
  influence_matrix = W_emp,    # empirical influence matrix (drives K_CC)
