...
| Evaluator discipline | Venture capital | Refactor candidate (time-varying $W$) | $K_{AC}$, $K_{CA}$, $K_{CC}$ |
...
K   <- make_epistasis_matrix(N, arch_type)      # modular | hierarchical | random
...
shift across waves.  \pkg{searchnet} presently accepts a fixed influence matrix;
admitting a wave-indexed array would let this analysis be written as an
\code{influence\_matrix} argument instead of hand-built covariate arrays, and
would additionally support any application where the technology landscape itself
evolves.
