...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $\bA \in \{0,1\}^{N \times N}$ is the epistasis matrix with
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item $N \in \N$ is the number of binary components.
  \item $\bE \in \{0,1\}^{N \times N}$ is the epistasis matrix (with
        $E_{dd}=1$ for all $d$).
...
other actors $\bB_{-i}$ and epistasis matrix $\bE$, is
\begin{equation}
\label{eq:saomnk_utility}
  u_i(\bb_i; \bB_{-i}, \bE)
  = \underbrace{\sum_{d=1}^{N} b_{id} \cdot
...
\begin{enumerate}[label=(R\arabic*)]
  ...
  \item\label{R2} $\bE = \bA$ (SaoMNK epistasis matrix equals the NK interaction
...
\label{eq:reduced_utility}
  u(\bx; \bE) = \sum_{d=1}^{N} x_d \cdot f_d\!\left(\bx \odot \bE_{d\cdot}\right).
\end{equation}
...
Now consider any configuration $\bx$ that the single actor would adopt. Since
the actor occupies exactly the components where $x_d = 1$, and the payoff
$f_d$ is only relevant for occupied components, we examine the index computation
\eqref{eq:powerkey_index}:
\begin{equation*}
  f_d\!\left(\bx \odot \bA_{d\cdot}\right)
  = \mathrm{NK\_land}\!\left[\sum_{j=1}^{N} x_j \, A_{dj} \, \mathrm{PK}(N)_j + 1,\; d\right].
\end{equation*}

In the standard NK model, $f_d(\bx_{\mathcal{N}(d)})$ uses the same indexing:
the sub-vector $\bx_{\mathcal{N}(d)}$ is converted to an integer via a power key
over the $K+1$ relevant bits. The SaoMNK formulation embeds this into the full
$N$-bit power key by zeroing out non-epistatic components via the mask
$A_{dj}$. Since $x_j \cdot A_{dj} = 0$ whenever $j \notin \mathcal{N}(d)$,
only the bits in $\mathcal{N}(d)$ contribute to the index sum, yielding the
identical table lookup.

Consider the case where $\bx = \mathbf{1}$ (all components set to 1, i.e., the
actor occupies all $N$ components). Then $x_d = 1$ for all $d$, and
\begin{equation*}
  u(\mathbf{1}; \bA) = \sum_{d=1}^{N} 1 \cdot f_d(\mathbf{1} \odot \bA_{d\cdot})
  = \sum_{d=1}^{N} f_d(\bx_{\mathcal{N}(d)}) = N \cdot W(\bx).
\end{equation*}

More generally, for the standard NK model to be fully embedded, we require
that the single actor always occupies all $N$ components (i.e.,
$\bx = \mathbf{1}$ as a constraint on the ``occupied'' set, with the
\emph{values} of the components---0 or 1---encoding the decision). In the NK
model, the state $\bx \in \{0,1\}^N$ represents the \emph{value} assigned to
each of $N$ components. In the SaoMNK bipartite representation, $b_{1d} = 1$
means actor~1 is connected to component $d$.

The equivalence is achieved by the following interpretation: each component $d$
in the NK model corresponds to a tie $b_{1d}$ in SaoMNK. The NK state
$x_d \in \{0,1\}$ maps directly to $b_{1d} \in \{0,1\}$. Thus, the NK
fitness at configuration $\bx$ equals:
\begin{equation}
\label{eq:payoff_equiv}
  W(\bx) = \frac{1}{N}\sum_{d=1}^{N} f_d(\bx_{\mathcal{N}(d)})
  = \frac{1}{N}\, u(\bx; \bA),
\end{equation}
since every term in \eqref{eq:reduced_utility} contributes (with $x_d$
serving as both the component value and the tie indicator). The factor
$\frac{1}{N}$ is a normalization constant that does not affect ordinal rankings
of configurations.
...
the epistasis matrix $\bA$. Other agents' configurations do not enter the
payoff function.
